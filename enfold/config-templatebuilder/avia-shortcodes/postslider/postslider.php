						...
						array(
							...
							'subtype'	=> array(
												...
												__( 'Title, Excerpt and Read More Link', 'avia_framework' )	=> 'excerpt_read_more',
          										__( 'Title, Excerpt Author, and Read More Link', 'avia_framework' )	=> 'excerpt_author_read_more',
		...
		public function html()
		{
			...
			foreach( $this->entries->posts as $index => $entry )
			{
				...
				$format = get_post_format( $the_id );
              	$testies = apply_filters('the_content', $entry->post_content);
              	$word_count = str_word_count(strip_tags($testies));
              	$reading_speed = 200;
				...
				else
				{
              	...
              	$author = get_field('author_bio', $entry->ID);
                if($author){
                  $authorthumbnail = get_the_post_thumbnail( $author->ID, 'medium', array( 'class' => 'authorthumb' ) );
                  $author_readmore = "<div class='author-post-link author-post-link-archive'>{$authorthumbnail}<div class='post-author-link-read-more'><a href='/bio/{$author->post_name}/' class='post-author-link'><span class='post-author-name'>{$author->post_title}</span></a><div class='post-read-more'>" . $reading_time . " min read</div></div></div>";
                }
				...
				$prepare_excerpt = ! empty( $entry->post_excerpt ) ? avia_backend_truncate( $entry->post_excerpt, apply_filters( 'avf_postgrid_excerpt_length', $excerpt_length ), apply_filters( 'avf_postgrid_excerpt_delimiter', ' ' ), '…', true, '' ) : avia_backend_truncate( $entry->post_content, apply_filters( 'avf_postgrid_excerpt_length', $excerpt_length ), apply_filters( 'avf_postgrid_excerpt_delimiter', ' ' ), '…', true, '' );
...
//							$meta_content .= '<div class="slide-meta-del">/</div>';
				...
				$output .= '</footer>';
              	...
              	if($contents == 'excerpt_author_read_more') {
                  if($author_readmore) {
                    $output .= $author_readmore;
                  }
              	}
